...
public class NestedPanel : MonoBehaviour {
   ...
   public void Merge(NestedPanel deadChild, Controller orphanedController) {
      ...
      livingChild.GetClientArea().StartDragTracking();
   ...
   private void RedrawBranch() {
      ...
      if (m_splitVertical) {
         ...
         secondTransform.sizeDelta = new Vector2(0, -rect.height * inverseSplitRatio);
   ...
   private void RedrawResizer() {
      ...
      if (m_splitVertical) {
         ...
         resizerTransform.anchoredPosition = new Vector2(0, rect.height * m_splitRatio);
